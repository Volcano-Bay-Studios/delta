...
public class LevelHandeler {
    ...
    public static void renderLevel() {
        ...
        int bottom = (int) RenderSystem.camera.unproject(new Vector3(0, Gdx.graphics.getHeight(), 0)).y-1;
        ...
        for (float i = start; i < (float) width; i += step) {
            ...
            for (float u = pos; u > bottom; u -= .1f){
                RenderSystem.batch.draw(SkyRenderer.star, i, u, 0, 0, 1, 1, .1f, .1f, 0, 0, 0, 1, 1, false, false);
            }
